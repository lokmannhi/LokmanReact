        ...
        .then((data) => {
          const reformatData = data.pokemon.map((x) => {
            ...
            return {
              ...
              url: x.pokemon.url,
              image_url: `https://raw.githubusercontent.com/PokeAPI/sprites/master/sprites/pokemon/${idx}.png`,
              third_gen: idx > 251 && idx <= 386 ? true : false
            };
          });

          setPokemon(reformatData.filter((x) => x.third_gen === true));
        })
        .catch((err) => {
          console.log(err);
        });
      ...
      <ol className="list">
        {pokemon.map((item) => (
          <li key={item.name} className="pokedex">
            ...
            <Link to={`/pokemon/hoeen/${item.name}`}>{item.name}</Link>
          </li>
        ))}
      </ol>
    </div>
